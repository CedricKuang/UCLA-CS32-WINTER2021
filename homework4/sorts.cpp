...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    for (int unsorted = 1; unsorted < SensorPtr.size(); unsorted++)
    {
        Sensor *nextItem = SensorPtr[unsorted];
        int loc = unsorted;
        while (loc > 0 && comp(nextItem, SensorPtr[loc - 1]))
        {
            *SensorPtr[loc - 1] = *SensorPtr[loc];
            loc--;
        }
        *SensorPtr[loc] = *nextItem;
    }

    for (int i = 0; i < sensors.size(); i++)
        sensors[i] = auxSensors[i];
