...
void City::preachToFlatulansAroundPlayer()
{
    ...
    {
        int rowdiff2 = m_flatulans[i]->row() - m_player->row();
        int coldiff2 = m_flatulans[i]->col() - m_player->col();
        if (rowdiff2 >= -1 && rowdiff2 <= 1 && coldiff2 >= -1 && coldiff2 <= 1)
        {
            if (m_history.record(m_player->row(), m_player->col()))
                break;
        }
    }
}
