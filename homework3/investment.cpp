/*#include <iostream>
#include <string>
using namespace std;*/

//The abstract base class Investment
class Investment
{
private:
    string m_name;
    int m_price;
public:
    Investment(string name, int price) : m_name(name), m_price(price) {};
    virtual ~Investment() {}        //virtual destrcutor
    string name() const { return m_name; }
    int purchasePrice() const { return m_price; }
    virtual bool fungible() const { return false; }        
    virtual string description() const = 0;     //pure virtual function
};

//class Painting inherited from Investment
class Painting : public Investment
{
public:
    Painting(string name, int price) : Investment(name, price) {};
    virtual ~Painting();
    virtual string description() const;
};

Painting::~Painting()
{
    cout << "Destroying " << name() << ", a painting" << endl;
}

string Painting::description() const
{
    return "painting";
}

//class Stock inherited from Investment
class Stock : public Investment
{
private:
    string m_tickersymbol;
public:
    Stock(string name, int price, string tickersymbol) : Investment(name, price), m_tickersymbol(tickersymbol) {}
    virtual ~Stock();   
    virtual bool fungible() const { return true; }
    virtual string description() const;
};

Stock::~Stock()
{
    cout << "Destroying " << name() << ", a stock holding" << endl;
}

string Stock::description() const
{
    return ("stock trading as " + m_tickersymbol);
}

//class House inherited from Investment
class House : public Investment
{
public:
    House(string name, int price) : Investment(name, price) {};
    virtual ~House();
    virtual string description() const;
};

House::~House()
{
    cout << "Destroying the house " << name() << endl;
}

string House::description() const
{
    return "house";
}


/*void display(const Investment* inv)
{
    cout << inv->name();
    if (inv->fungible())
        cout << " (fungible)";
    cout << ": " << inv->description() << " bought for $" << inv->purchasePrice() << endl;
}

int main()
{
    Investment* portfolio[4];
    portfolio[0] = new Painting("Salvator Mundi", 450300000);
    // Stock holdings have a name, value, and ticker symbol
    portfolio[1] = new Stock("Lyft", 50000, "LYFT");
    portfolio[2] = new Stock("GameStop", 10000, "GME");
    portfolio[3] = new House("4 Privet Drive", 660000);

    for (int k = 0; k < 4; k++)
        display(portfolio[k]);

    // Clean up the investments before exiting
    cout << "Cleaning up" << endl;
    for (int k = 0; k < 4; k++)
        delete portfolio[k];
}*/